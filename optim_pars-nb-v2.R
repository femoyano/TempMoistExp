...
pars_optim_init <- c(
  ...
  # For all cases
  f_CA_bf   = 0.3    ,  # fraction of C_A in TOC for bare fallow
  f_CA_mz   = 0.3    ,  # fraction of C_A in TOC for maize soil 
  ...
  f_ep      = 0.2   ,  # fraction of uptaken C converted to C_E (assumed)
  V_D_ref   = 0.1      ,  # [h-1] max rate of C_P decomposition (Li at al. 2014, AWB model)
  V_U_ref   = 1      ,  # [h-1] max rate of microbial C uptake (assumed
  K_D_ref   = 50000  ,  # [gC m-3] Affinity parameter for C_P decomp. Guessed starting value.
  K_U_ref   = 10     ,  # [gC m-3] Affinity parameter for C_D uptake.
  r_ed_ref  = 0.001  ,  # [h-1] Enzyme turnover rate (Li at al. 2014, AWB model).
  E_V       = 90     ,  # [kJ mol^-1] Gibbs energy for decomposition and turnover times
  E_K       = 90     ,  # [kJ mol^-1] Gibbs energy for K_D
  psi_Rth   = 16000  ,  # [kPa] Threshold water potential for microbial respiration (Manzoni and Katul 2014)
  D_0       = 0.4      # [m s-1] reference diffusion conductance for dissolved C (and /10 for enzymes), representing diffusivity/distance.  
)
