...
pars_optim_lower <- c(
  ...
  #   f_CEm     = 0.0001 , #
  #   f_CEw     = 0.0001 , #
  r_ed_ref  = 0 / hour * tstep , # [h-1] Enzyme turnover rate (Li at al. 2014, AWB model).
  V_D_ref   = 0   / hour * tstep , # [h-1] Maximum speed of C_P decomposition (Li at al. 2014, AWB model)
  K_D_ref   = 0   , # [gC m-3] Affinity parameter for C_P decomp. (Adjusted. As ref: Li at al. 2014, AWB model => 250 mg gSoil-1)
  E_V       = 0     , # [kJ mol^-1] Gibbs energy for decomposition and turnover times
  E_K       = 0     , # [kJ mol^-1] Gibbs energy for K_D
  f_gr_ref  = 0    , # Carbon use efficieny (= microbial growth efficiency) (Hagerty et al.)
  f_de      = 0  , # [g g-1] Fraction of C_D taken up that is converted to C_E (fitted).
  psi_Rth   = 0  , # [kPa] Threshold water potential for microbial respiration (Manzoni and Katul 2014)
  D_0      =  0 / sec  * tstep # [m s-1] reference diffusion conductance for dissolved C (and /10 for enzymes), representing diffusivity/distance.
...
pars_optim_upper <- c(
  ...
  #   k_des_ref = 1.2e-8 / sec * tstep , # [s-1] Desorption rate constant of C_A. (Ahrens 2015)
